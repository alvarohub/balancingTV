...
ofPoint ofxOpticalFlowLK :: getMeanFlow ( int width, int height, int res )
{
	bool rightSize = ( sizeSml.width == width && sizeSml.height == height );
	
    ofPoint meanVel=ofPoint(0,0,0); 
	...
	for( int x=0; x<width; x+=res )
	{
		...
		{
			if( rightSize )
				meanVel+=getVelAtPixel( x, y );
			else
				meanVel+=getVelAtNorm( x / (float)width, y / (float)height );
		}
	}
    
    meanVel/=(width/res)*(height/res);
    return(meanVel);
}
